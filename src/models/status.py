...
class Status(Table):
    ...
    def find_all_by_id_user(self,id_user):
        query = f"""
                ...
                """
        result = self.select(query)
        status_list = [Status(status['id'],status['id_user'],status['name']) for status in result]
        if len(status_list) == 0:
            raise ToDoException("wasn't able to find any status related to this user",404)
        return status_list
